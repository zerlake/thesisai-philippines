      ...
      create: ({ messages }: { messages: Array<{ role: string; content: string }> }) => {
        ...
        if (userMessage && userMessage.content.includes('test topic')) {
          return Promise.resolve({
            choices: [{
              message: {
                content: JSON.stringify({ flashcards: [{ term: "Test Term 1", definition: "Definition 1" }, { term: "Test Term 2", definition: "Definition 2" }] }),
              },
            }],
          });
        }
          ...
          choices: [{
            message: {
              content: JSON.stringify({
                flashcards: [
                  { term: "Default Term 1", definition: "Default Definition 1" },
                  { term: "Default Term 2", definition: "Default Definition 2" }
                ]
              }),
            },
          }],
...
async function simulateRequest(body: any, headers: Record<string, string> = {}) {
  const mockRequest = new Request('http://localhost/generate-flashcards', {
    method: 'POST',
    headers: {
...
Deno.test("generate-flashcards: should return generated flashcards", async () => {
  const response = await simulateRequest({ topic: "This is a test topic." });
  const flashcardData = await response.json();
  assertEquals(response.status, 200);
  assertEquals(flashcardData.flashcards.length, 2);
  assertEquals(flashcardData.flashcards[0].term, "Test Term 1");
});

Deno.test("generate-flashcards: should return 400 if topic is missing", async () => {
...
Deno.test("generate-flashcards: should return 500 if authorization header is missing", async () => {
...
Deno.test("generate-flashcards: should return 500 if OpenRouter API fails", async () => {
