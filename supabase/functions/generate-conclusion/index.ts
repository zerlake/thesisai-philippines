...
async function generateConclusionWithOpenRouter(findings: string, apiKey: string) {
  const prompt = `You are an expert academic writing assistant. Your task is to write the key sections of a thesis's final chapter (Chapter V) based on a provided summary of research findings.
...
serve(async (req: Request) => {
  ...
  try {
    ...
    const { findings } = await req.json() as RequestBody;
    if (!findings) {
    ...
    const conclusionData = await generateConclusionWithOpenRouter(findings, openrouterApiKey);
    ...
    console.error("Error in generate-conclusion function (OpenRouter):", error);
