-- Phase 4.1: Database Schema Creation, RLS Security Policies, Performance Indexing
...
-- Update the profiles table to use trigger instead of function in INSERT
-- This avoids issues with auth.uid() in stored functions
...
CREATE INDEX IF NOT EXISTS profiles_university_idx ON profiles USING btree(university_id);
CREATE INDEX IF NOT EXISTS profiles_role_idx ON profiles USING btree(role);

-- Institutions table indexes
CREATE INDEX IF NOT EXISTS institutions_slug_idx ON institutions USING btree(slug);
CREATE INDEX IF NOT EXISTS institutions_type_idx ON institutions USING btree(type);
CREATE INDEX IF NOT EXISTS institutions_location_idx ON institutions USING btree(location);

-- Thesis projects table indexes
CREATE INDEX IF NOT EXISTS thesis_projects_user_id_idx ON thesis_projects USING btree(user_id);
CREATE INDEX IF NOT EXISTS thesis_projects_advisor_id_idx ON thesis_projects USING btree(advisor_id);
CREATE INDEX IF NOT EXISTS thesis_projects_status_idx ON thesis_projects USING btree(status);
CREATE INDEX IF NOT EXISTS thesis_projects_academic_year_idx ON thesis_projects USING btree(academic_year);

-- Thesis documents table indexes
CREATE INDEX IF NOT EXISTS thesis_documents_project_id_idx ON thesis_documents USING btree(project_id);
CREATE INDEX IF NOT EXISTS thesis_documents_user_id_idx ON thesis_documents USING btree(user_id);
CREATE INDEX IF NOT EXISTS thesis_documents_type_idx ON thesis_documents USING btree(type);
CREATE INDEX IF NOT EXISTS thesis_documents_status_idx ON thesis_documents USING btree(status);
CREATE INDEX IF NOT EXISTS thesis_documents_locked_by_idx ON thesis_documents USING btree(locked_by);

-- Advisor feedback table indexes
CREATE INDEX IF NOT EXISTS advisor_feedback_document_id_idx ON advisor_feedback USING btree(document_id);
CREATE INDEX IF NOT EXISTS advisor_feedback_reviewer_id_idx ON advisor_feedback USING btree(reviewer_id);
CREATE INDEX IF NOT EXISTS advisor_feedback_student_id_idx ON advisor_feedback USING btree(student_id);
CREATE INDEX IF NOT EXISTS advisor_feedback_resolved_idx ON advisor_feedback USING btree(resolved);

-- Activity logs table indexes
CREATE INDEX IF NOT EXISTS activity_logs_user_id_idx ON activity_logs USING btree(user_id);
CREATE INDEX IF NOT EXISTS activity_logs_entity_type_id_idx ON activity_logs USING btree(entity_type, entity_id);
...
CREATE INDEX IF NOT EXISTS ai_tool_usage_user_id_idx ON ai_tool_usage USING btree(user_id);
CREATE INDEX IF NOT EXISTS ai_tool_usage_tool_name_idx ON ai_tool_usage USING btree(tool_name);
CREATE INDEX IF NOT EXISTS ai_tool_usage_created_at_idx ON ai_tool_usage USING btree(created_at DESC);

-- Originality checks table indexes
CREATE INDEX IF NOT EXISTS originality_checks_document_id_idx ON originality_checks USING btree(document_id);
CREATE INDEX IF NOT EXISTS originality_checks_user_id_idx ON originality_checks USING btree(user_id);
CREATE INDEX IF NOT EXISTS originality_checks_status_idx ON originality_checks USING btree(status);

-- Enable pg_trgm extension for fuzzy matching
CREATE EXTENSION IF NOT EXISTS pg_trgm;

-- Additional trigram indexes for fuzzy text search
CREATE INDEX IF NOT EXISTS profiles_first_name_trgm ON profiles USING gin(first_name gin_trgm_ops);
CREATE INDEX IF NOT EXISTS profiles_last_name_trgm ON profiles USING gin(last_name gin_trgm_ops);
CREATE INDEX IF NOT EXISTS institutions_name_trgm ON institutions USING gin(name gin_trgm_ops);
CREATE INDEX IF NOT EXISTS thesis_projects_title_trgm ON thesis_projects USING gin(title gin_trgm_ops);

-- Text search indexes for common searchable fields
CREATE INDEX IF NOT EXISTS thesis_projects_title_search ON thesis_projects USING gin(to_tsvector('english', title || ' ' || COALESCE(subtitle, '') || ' ' || COALESCE(abstract, '')));
CREATE INDEX IF NOT EXISTS topic_ideas_topic_description_search ON topic_ideas USING gin(to_tsvector('english', topic || ' ' || COALESCE(description, '')));
