...
import { Minimize, Eye, CheckCircle, XCircle } from "lucide-react";
...
export function Editor({ documentId }: { documentId: string }) {
  ...
  // Ref to track if content is being set programmatically to avoid update loops
  ...
  const editor = useEditor({
    ...
    onUpdate({ editor }) {
      // Only update document content if this update is from user input, not from programmatic sync
  ...
  // Sync editor content with document data, but prevent triggering updates
  useEffect(() => {
    if (editor && doc.content && editor.getHTML() !== doc.content) {
      isSettingContentRef.current = true;
      editor.commands.setContent(doc.content, { emitUpdate: false });
      // Reset the flag after a brief moment to allow user updates again
      setTimeout(() => {
