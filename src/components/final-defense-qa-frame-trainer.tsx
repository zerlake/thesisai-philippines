...
// Main dashboard integrating all four Defense Framework Trainers
...
export default function DefensePreparationPage() {
  ...
  const [frameworkProgress, setFrameworkProgress] = useState<FrameworkProgress[]>([
    { framework: "General Q&A", progress: 75, questionsAnswered: 15, avgConfidence: 65 },
    { framework: "Title Defense", progress: 45, questionsAnswered: 8, avgConfidence: 55 },
    { framework: "Proposal Q&A", progress: 60, questionsAnswered: 12, avgConfidence: 70 },
    { framework: "Final Defense", progress: 30, questionsAnswered: 6, avgConfidence: 60 }
  ]);
  ...
  const resetPractice = () => {
    ...
    setFrameworkProgress([
      { framework: "General Q&A", progress: 0, questionsAnswered: 0, avgConfidence: 50 },
      { framework: "Title Defense", progress: 0, questionsAnswered: 0, avgConfidence: 50 },
      { framework: "Proposal Q&A", progress: 0, questionsAnswered: 0, avgConfidence: 50 },
      { framework: "Final Defense", progress: 0, questionsAnswered: 0, avgConfidence: 50 }
    ]);
    setStressLevel(50);
  ...
  const handleSaveAsDraft = async () => {
    if (!user || !frameworkProgress.some(fp => fp.progress > 0)) return;
    setIsSaving(true);

    let content = `<h1>Defense Preparation Summary</h1><hr>`;
    
    content += `<h2>Statistics</h2>
      <ul>
        <li>Total Sessions: ${stats.totalSessions}</li>
        <li>Average Confidence: ${stats.avgConfidence}%</li>
        <li>Frameworks Mastered: ${stats.frameworksMastered}/4</li>
        <li>Practice Questions: ${stats.practiceQuestions}</li>
        <li>Streak Days: ${stats.streakDays}</li>
      </ul>`;
    
    if (frameworkProgress.length > 0) {
      content += `<hr><h2>Framework Progress</h2><ul>`;
      frameworkProgress.forEach(fp => {
        content += `<li>${fp.framework}: ${fp.progress}% (${fp.questionsAnswered} questions, ${fp.avgConfidence}% confidence)</li>`;
      });
      content += `</ul>`;
    }
    
    if (stats.achievements.length > 0) {
      content += `<hr><h2>Achievements</h2><ul>`;
    ...
    content += `<p>Next Milestone: ${stats.nextMilestone}</p>`;

    const { data: newDoc, error } = await supabase
      .from("documents")
      .insert({
        user_id: user.id,
        title: "Defense Preparation Summary",
        content: content,
      })
      .select("id")
      .single();
    ...
    if (error) {
      ...
      toast.success("Defense preparation summary saved as a new draft!");
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          <div className="flex flex-wrap gap-2">
            ...
            <Button variant="outline" onClick={handleStartTitlePractice}>
              <Target className="w-4 h-4 mr-2" />
              Title Defense Practice
            </Button>
            <Button variant="outline" onClick={handleStartProposalPractice}>
              <FilePlus2 className="w-4 h-4 mr-2" />
              Proposal Q&A Practice
            </Button>
            <Button variant="outline" onClick={handleStartDefensePractice}>
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <div className="text-2xl font-bold">{stats.frameworksMastered}/4</div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  <div>
                    ...
                    <div className="space-y-4">
                      {frameworkProgress.map((fp, index) => (
                        <div key={index} className="space-y-2">
                          <div className="flex justify-between">
                            <span className="font-medium">{fp.framework}</span>
                            <span>{fp.progress}%</span>
                          </div>
                          <Progress value={fp.progress} className="h-2" />
                          <div className="flex justify-between text-xs text-muted-foreground">
                            <span>{fp.questionsAnswered} questions answered</span>
                            <span>Avg. confidence: {Math.round(fp.avgConfidence)}%</span>
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                      ...
                      >
                        <CardContent className="p-4 text-center">
                          ...
                          <Badge variant="secondary" className="mt-2">
                            {frameworkProgress.find(fp => fp.framework === "General Q&A")?.progress || 0}% Complete
                          </Badge>
                        </CardContent>
                      </Card>
                      ...
                      <Card 
                        className="cursor-pointer transition-all hover:shadow-md hover:border-primary"
                        onClick={handleStartTitlePractice}
                      >
                        <CardContent className="p-4 text-center">
                          ...
                          <Badge variant="outline" className="mt-2">
                            {frameworkProgress.find(fp => fp.framework === "Title Defense")?.progress || 0}% Complete
                          </Badge>
                        </CardContent>
                      </Card>
                      ...
                      <Card 
                        className="cursor-pointer transition-all hover:shadow-md hover:border-primary"
                        onClick={handleStartProposalPractice}
                      >
                        <CardContent className="p-4 text-center">
                          ...
                          <Badge variant="outline" className="mt-2">
                            {frameworkProgress.find(fp => fp.framework === "Proposal Q&A")?.progress || 0}% Complete
                          </Badge>
                        </CardContent>
                      </Card>
                      ...
                      <Card 
                        className="cursor-pointer transition-all hover:shadow-md hover:border-primary"
                        onClick={handleStartDefensePractice}
                      >
                        <CardContent className="p-4 text-center">
                          ...
                          <Badge variant="outline" className="mt-2">
                            {frameworkProgress.find(fp => fp.framework === "Final Defense")?.progress || 0}% Complete
                          </Badge>
                        </CardContent>
                      </Card>
                    </div>
