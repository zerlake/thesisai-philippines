-- ============================================
-- REVIEW NOTES TABLE FOR ADVISOR/CRITIC FEEDBACK
-- Run this SQL in Supabase Dashboard SQL Editor
-- https://supabase.com/dashboard/project/dnyjgzzfyzrsucucexhy/sql/new
-- ============================================
...
DROP POLICY IF EXISTS "Reviewers can view their own notes" ON public.review_notes;
...
DROP POLICY IF EXISTS "Reviewers can create notes" ON public.review_notes;
...
DROP POLICY IF EXISTS "Reviewers can update their own notes" ON public.review_notes;
...
DROP POLICY IF EXISTS "Reviewers can delete their own notes" ON public.review_notes;
...
DROP POLICY IF EXISTS "Students can view sent notes on their documents" ON public.review_notes;
CREATE POLICY "Students can view sent notes on their documents"
    ...
    USING (
        status = 'sent'
        AND document_id IN (
            SELECT id FROM public.documents WHERE user_id::uuid = auth.uid()
        )
    );
